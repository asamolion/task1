...
class CSV:
    ...
    def get_data(self, *args, **kwargs):
        ...
        for arg in args:
            ...
            max_value = max(record.data_dict['max_' + arg]
                            for record in self.record_list)
            max_date = max(record.date for record in self.record_list
                           if record.data_dict['max_' + arg] == max_value)
            mean_value = int(mean(list(record.data_dict['mean_' + arg]
                                       for record in self.record_list)))
            min_value = min(record.data_dict['min_' + arg]
                            for record in self.record_list)
            min_date = min(record.date for record in self.record_list
                           if record.data_dict['min_' + arg] == min_value)
            ...
            the_file.write("Minimum temperature was {:d} on {}\n".format(
                min_value, min_date))
            #######################################################
            max_list = [record.data_dict['max_' + arg]
                        for record in self.record_list]
            min_list = [record.data_dict['min_' + arg]
                        for record in self.record_list]
                ...
                if int(value_date[1]) == 1:
                    the_file.write("{0}: on {1}\n".format(
                        value_date[0], max(record.date for record in self.record_list
                                           if record.data_dict['max_' + arg] == value_date[0])))
                else:
            ...
            for i in range(10):
                ...
                if int(value_date[1]) == 1:
                    the_file.write("{0}: on {1}\n".format(
                        value_date[0], max(record.date for record in self.record_list
                                           if record.data_dict['min_' + arg] == value_date[0])))
                else:
                    the_file.write("{0}: on {1}\n".format(
                        value_date[0], value_date[1]))
            the_file.write('\n')
